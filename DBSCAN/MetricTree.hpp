...
public:
    ...
    void search(const T &target, const double radius , std::vector<std::vector<double>>& result);
};
...
template<typename T, double(*distance)(const T &, const T &)>
void MetricTree<T, distance>::search(const T &target, const double radius , std::vector<std::vector<double>>& result) {
